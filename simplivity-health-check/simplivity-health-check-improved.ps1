...
$script:WINDOW_WIDTH = 214
$script:WINDOW_HEIGHT = 50
$script:BUFFER_HEIGHT = 8000
...
# SFTP Configuration
$script:SFTP_SERVER = "hprc-h2.it.hpe.com"
$script:SFTP_PORT = 2222

# Thresholds
$script:MAX_CLUSTER_MEMBERS = 16
$script:MIN_FREE_CAPACITY_PERCENT = 20
$script:CPU_USAGE_THRESHOLD = 80
$script:MEMORY_USAGE_THRESHOLD = 80
$script:HIGH_USAGE_THRESHOLD = 85

# Timeouts (in seconds)
$script:SSH_COMMAND_TIMEOUT = 60
$script:NETWORK_TEST_TIMEOUT = 10
$script:SUPPORT_DUMP_WAIT_TIME = 300

# Snapshot Configuration
$script:SNAPSHOT_AGE_DAYS = 3

# File Paths
$script:INFRA_VARIABLE_FILE = "$PSScriptRoot\infra_variable.json"
$script:CREDENTIAL_FILE = "$PSScriptRoot\cred.XML"
$script:SFTP_CREDENTIAL_FILE = "$PSScriptRoot\sftp-cred.XML"
$script:REPORT_DIR_PATH = "$PSScriptRoot\Reports"

# Java Version Requirements
$script:MIN_JAVA_VERSION = '8.0'
$script:MAX_JAVA_VERSION = '9.0'
$script:MIN_ASPNET_VERSION = '4.7'

# Update Manager OS Builds
$script:SUPPORTED_OS_BUILDS = @('6003', '7601', '9200', '9600', '14393', '16299', '17134', '17763', '18362', '18363', '19041', '19042', '20348')
$script:MIN_MEMORY_GB = 14
$script:MIN_CPU_CORES = 2
#endregion Constants
...
$newsize.Width = $script:WINDOW_WIDTH
$newsize.Height = $script:BUFFER_HEIGHT
$pswindow.buffersize = $newsize

$newsize = $pswindow.windowsize
$newsize.Width = $script:WINDOW_WIDTH
$newsize.Height = $script:WINDOW_HEIGHT
...
function Test-Net-Connection($destination)  {
 ...
 function Get-SVT-Cluster {
	 ...
	 Test-Net-Connection $global:vCenterServer
			 ...
			 if ($ovcid -ge 2 )
			 {
	            ...
	            Test-Net-Connection $selectedovcIpAddress
				 ...
				 try {
					 ...
					 $OVCSSHConnection = New-SSHSession -ComputerName $selectedovcIpAddress -port $script:SSH_PORT -Credential $Cred -AcceptKey -ErrorAction Stop
			 ...
			 # Validate cluster state and omnistack_clusters array
			 if ($null -eq $clusterstate -or $null -eq $clusterstate.omnistack_clusters -or $clusterstate.omnistack_clusters.Count -eq 0) {
				 Write-Host "Error: Unable to retrieve cluster state or cluster data is empty." -ForegroundColor Red
				 Break
			 }

			 # Store the first cluster for easier access and better null safety
			 ...
			 $vmhastate = $null
			 $CLSReportFile = "$($global:ReportDirPath)\$($currentCluster.name)-$($logtimestamp).log"
			 ...
			 $vmwarecluster = Get-Cluster -Name $currentCluster.name
			 ...
			 Write-Host "`nDatacenter Name:                  $($currentCluster.hypervisor_object_parent_name)"
			 Write-Host "Cluster Name:                     $($currentCluster.name)"
			 Write-Host "Hypervisor Type:                  $($currentCluster.hypervisor_type)"
			 Write-Host "Management System:                $($currentCluster.hypervisor_management_system_name)"	
			 
			 if ($currentCluster.members.Count -lt $script:MAX_CLUSTER_MEMBERS) {
					 Write-Host "SVT Cluster Members Count:        $($currentCluster.members.Count)"

			 } else {
					 Write-Host "SVT  Cluster Members Count:        $($currentCluster.members.Count)" -ForegroundColor Red
					 $memberscount = 1
			 }
			 Write-Host "SVT Current Running Version:      $($currentCluster.version)"
			 if ($currentCluster.upgrade_state -eq 'SUCCESS_COMMITTED') {
					 Write-Host "SVT Cluster Ver. Upgrade State:   $($currentCluster.upgrade_state)"
			 
			 } else {
					 Write-Host "SVT Cluster Ver. Upgrade State:   $($currentCluster.upgrade_state)" -ForegroundColor Red
					 $upgradestate = 1
			 }				 
			 Write-Host "SVT Time Zone:                    $($currentCluster.time_zone)"
			 ...
			 if ($clustercpuusage.Average -gt $script:CPU_USAGE_THRESHOLD) {
              ...
              if ($clustermemusage.Average -gt $script:MEMORY_USAGE_THRESHOLD) {
			 ...
			 $pysical_space = $($currentCluster.allocated_capacity / 1TB).ToString("F2")
			 $used_space = $($currentCluster.used_capacity / 1TB).ToString("F2")
			 $free_space = $($currentCluster.free_space / 1TB).ToString("F2")
			 $local_backup_space = $($currentCluster.local_backup_capacity / 1TB).ToString("F2")
			 $percentFree = $(($currentCluster.free_space / $currentCluster.allocated_capacity) * 100).ToString("F2")	
			 $efficiency_ratio = $($currentCluster.efficiency_ratio) 
			 ...
			 if ($percentFree -ge $script:MIN_FREE_CAPACITY_PERCENT) {
			 ...
			 # Get SVT Host Status
			 $hostlist = Get-Cluster -Name $currentCluster.name | Get-VMHost
			 # Create a table to display svt host information
			 $HostTable = [System.Collections.ArrayList]::new()
			 ...
			 ## SVT Host Alarms
             $VMHAlarmReport = [System.Collections.ArrayList]::new()
             ...
             if ($HostErrors){
	             foreach ($HostError in $HostErrors){
		              foreach($alarm in $HostError.TriggeredAlarmState){
			             ...
			             [void]$VMHAlarmReport.Add((New-Object PSObject -Property $Hprops))
		              }
	            }
	        }
             ...
             $VCAlertsTable = [System.Collections.ArrayList]::new()
             ...
             if ($VCAlerts) {
				 ...
				 foreach ($VCAlertsDetail in $VCAlerts) {
					 ...
					 [void]$VCAlertsTable.Add($VCAlertsInfo)
				} 
			 ...
			 Write-Host "`nRequired Arbiter:   $($currentCluster.arbiter_required)"
			 
			 if ($currentCluster.arbiter_required -eq 'true') {
				 if ($currentCluster.arbiter_configured -eq 'true') {
						 Write-Host "Arbiter Configured:   $($currentCluster.arbiter_configured)"
						 
						 if ($currentCluster.arbiter_connected -eq 'true') {
								 Write-Host "Arbiter Conected:   $($currentCluster.arbiter_connected)"
								 Write-Host "Arbiter Address:   $($currentCluster.arbiter_address)"
							 }else {
								 Write-Host "Arbiter Conected:   $($currentCluster.arbiter_connected)" -ForegroundColor Red
								 $arbiterconnected = 1 
							 }
	             
				 } else {
						 Write-Host "Arbiter Configured  :   $($currentCluster.arbiter_configured)" -ForegroundColor Red
						 $arbiterconfigured = 1
				 }
			 ...
			 $SvtIntWork = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtIntWorkCmd  -TimeOut $script:SSH_COMMAND_TIMEOUT
			 $SvtIntWork.Output  
			 
			 # Get SVT Datastore Status
			 Write-Host "`n"
			 Write-Host "`n### Simplivity Datastore List ###" -ForegroundColor White
			 $DSDetailList = Get-SvtDatastore | Where-Object  ClusterName -eq $currentCluster.name |  Select-Object DatastoreName, SizeGB, SingleReplica, ClusterName, Deleted, PolicyName
			 $DatastoreTable = [System.Collections.ArrayList]::new()
			 ...
			 foreach ($DSDetail in $DSDetailList) {
				 ...
				 [void]$DatastoreTable.Add($dsInfo)
			 }
			 ...
			 $VMDetailList = Get-SvtVM -ClusterName $currentCluster.name
			 # Create a table to display virtual machine information
			 $VMTable = [System.Collections.ArrayList]::new()

			 foreach ($VMDetail in $VMDetailList) {

				 $VMBackup = Get-SvtBackup -DestinationName $currentCluster.name -VmName $VMDetail.VmName -All -ErrorAction SilentlyContinue
			 ...
			 ## Active VM Alarms
             $VMAlarmReport = [System.Collections.ArrayList]::new()
             ...
             if ($VMErrors) {
                  foreach ($VMError in $VMErrors){
                      foreach ($TriggeredAlarm in $VMError.TriggeredAlarmstate) {
                        ...
                        [void]$VMAlarmReport.Add((New-Object PSObject -Property $VMprops))
                      }
                 }
            }
            ...
            # Snapshot Report
			$snapshotdays = $script:SNAPSHOT_AGE_DAYS
            $Snapshotdate = (Get-Date).AddDays(-$snapshotdays)
            $VMSnapshotReport = [System.Collections.ArrayList]::new()
            $SnapshotCmd  = Get-Cluster -Name $currentCluster.name | Get-VM | get-snapshot
            ...
            if ($SnapshotDateReport){
                foreach ($snapshot in $SnapshotDateReport) {
			        ...
			        [void]$VMSnapshotReport.Add($SnapInfo)
	            }
            }
            ...
            if ($SnapshotDateReport) {
			 ...
			 $BackupRulesTable = [System.Collections.ArrayList]::new()
			 ...
			 foreach ($BackupPolicy in $BackupPolicies.policies) {
				foreach ($rule in $BackupPolicy.rules) {
					...
					[void]$BackupRulesTable.Add($BackupRuleData)
				}
			 }
			 ...
			 $SvtBackupReport = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtBackupReportCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
			 ...
			 $SvtBackupQueue = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtBackupQueueCmd  -TimeOut $script:SSH_COMMAND_TIMEOUT
			 ...
			 $SvtExternalStore = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtExternalStoreCmd  -TimeOut $script:SSH_COMMAND_TIMEOUT
			 ...
			 $SvtBalanceNode = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtBalanceNodeCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
			 ...
			 $SvtBalance = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtBalanceCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
			 $SvtBalance.Output	
			 ...
			 $SvtBroadcast = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtBroadcastCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
			 $SvtBroadcast.Output	
			 ...
			 $SvtElection = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtElectionCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
			 $SvtElection.Output	
			 
			 # Get Virtual Machine Replica Sets
			 Write-Host "`n"
			 Write-Host "`n### Virtual Machine Replica Sets  ###" -ForegroundColor White
			 $SvtVMReplicaSet = Get-SVTvmReplicaSet -ClusterName $currentCluster.name
			 # Create a table to display virtual machine replicaset information
			 $ReplicaSetTable = [System.Collections.ArrayList]::new()
			 ...
			 foreach ($ReplicaDetail in $SvtVMReplicaSet) {
				 ...
				 [void]$ReplicaSetTable.Add($ReplicaInfo)
			 }
			 ...
			 $ReplicaStateTable = [System.Collections.ArrayList]::new()
             foreach ($VMList in $VMDetailList) {
                 ...
                 if ($VMList.Hostname -ne $VMReplica.Primary) {
	                  ...
	                  [void]$ReplicaStateTable.Add($ReplicaState)
	            }
            } 
			 ...
			 $SvtSupport = Invoke-SSHcommand -SessionId $SSHOVCSession.SessionID -Command $SvtSupportCmd  -TimeOut $script:SSH_COMMAND_TIMEOUT
			 $SvtSupport.Output
			 ...
			 if ($upgradestate -eq $null -and $memberscount -eq $null -and $arbiterconfigured -eq $null -and $arbiterconnected -eq $null -and $storagefreestate -eq $null -and $vmreplicasetdegreded.Count -eq 0 -and $vmclsstate -eq $null -and $vmhastate -eq $null) {
					 Write-Host "`nMessage: The status of the cluster ($($currentCluster.name)) is consistent and you can continue to upgrade .... " -ForegroundColor Green
			 } else {
				 
				 Write-Host "`nMessage: SVT cluster ($($currentCluster.name)) status is not consistent and should fix error states !!! " -ForegroundColor Red
				 
				 if ($upgradestate) {
				 Write-Host "`nError Message: Update status not in the expected state !!! "  -ForegroundColor Red
				 }
				 if ($memberscount) {
				 Write-Host "`nError Message: Svt cluster ($($currentCluster.name)) is comprised of more than 16 HPE OmniStack hosts !!!"  -ForegroundColor Red
				 }
			 ...
			 $hoststate = Get-SvtHost -ClusterName $currentCluster.name -Raw | ConvertFrom-Json 
			 ...
			 foreach ($svthost in $hoststate.hosts) {
	            ...
	            Test-Net-Connection $svthost.management_ip
				 ...
				 try {
					 ...
					 $SSHOVCConnection = New-SSHSession -ComputerName $svthost.management_ip -port $script:SSH_PORT -Credential $Cred -AcceptKey -ErrorAction Stop
				 ...
				 if ($svthost.version -eq $currentCluster.version) {
				 ...
				 $shutdownstate = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $shutdownstateCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
				 ...
				 if ($percentCpu -le $script:HIGH_USAGE_THRESHOLD) {
				 ...
				 if ($percentMem -le $script:HIGH_USAGE_THRESHOLD) {
				 ...
				 ## SVT Host Alarms
                 $SVTAlarmReport = [System.Collections.ArrayList]::new()
                ...
                if ($SVTHostErrors){
	                foreach ($SVTHostError in $SVTHostErrors){
		                 foreach($svtalarm in $SVTHostError.TriggeredAlarmState){
			                 ...
			                 [void]$SVTAlarmReport.Add((New-Object PSObject -Property $SVTHprops))
		                 }
	                }
	             }
				...
				$NetState = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $NetStateCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
				...
				$NetTest = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $NetTestCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
				...
				$cfgdbstate = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $cfgdbstateCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
				...
				$nodecompliance = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $nodecomplianceCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
				...
				$watchdogstate = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $watchdogstateCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
				...
				$ServiceState = Invoke-SSHcommand -SessionId $OVCSession.SessionID -Command $ServiceStateCmd -TimeOut $script:SSH_COMMAND_TIMEOUT
                ...
                $OVCAlarms = [System.Collections.ArrayList]::new()
                ...
                if ($OVCErrors) {
                      foreach ($OVCError in $OVCErrors){
                          foreach ($OVCTriggeredAlarm in $OVCError.TriggeredAlarmstate) {
                            ...
                            [void]$OVCAlarms.Add((New-Object PSObject -Property $OVCprops))
                          }
                     }
                }
			 ...
			 try {
				 ...
				 $SSHCaptureCon = New-SSHSession -ComputerName $selectedovcIpAddress -port $script:SSH_PORT -Credential $global:Cred -ErrorAction Stop
				 Write-Host "Connection established to target OVC Host - $($selectedovcIpAddress) `n" -ForegroundColor Green
			 } catch {
			 ...
			 try {
				 ...
				 $null = Invoke-SSHcommand -SessionId $CaptureSession.SessionId -Command $sshbalance -TimeOut $script:SSH_COMMAND_TIMEOUT  -ErrorAction Stop
		...
		try{	 
			 ...
			 if ($ovcid -ge 2 ){
	            ...
	            Test-Net-Connection $selectedovcIpAddress
				 ...
				 try {
					 # Attempt to access  OVC IP address in the array    
					 $SSHDumpCon = New-SSHSession -ComputerName $selectedovcIpAddress -port $script:SSH_PORT -Credential $Cred -ErrorAction Stop
					 Write-Host "Connection established to target OVC Host - $($selectedovcIpAddress) `n" -ForegroundColor Green
				 } catch {
...
function Upload-Report-Files{
	...
	# Define Server Name
	$SFTPServer = $script:SFTP_SERVER
	$SFTPServerPort = $script:SFTP_PORT
	...
	try {
		...
		$Global:ProgressPreference = 'SilentlyContinue'
		$SFTPconnection = Test-NetConnection -Port $script:SFTP_PORT -ComputerName $SFTPServer -ErrorAction stop -WarningAction Stop
...
function Get-Update-Manager{
	 ...
	 Clear-Host
		  ...
		  # Check Host Status
		  $javaInfo = Get-CimInstance -Query "SELECT * FROM Win32_Product WHERE (Name LIKE 'Java%') AND NOT (Name LIKE '%Updater%')"
		  ...
		  if ($javaInfo) {
			 ...
			 $javaVersion = $javaInfo | Where-Object { $_.Version -ge $script:MIN_JAVA_VERSION -and $_.Version -lt $script:MAX_JAVA_VERSION }
		  ...
		  # Check Host Status
		  $AspNetInfo = Get-CimInstance -Query "SELECT * FROM Win32_Product WHERE (Name LIKE 'Microsoft%ASP.NET%Core%')"
